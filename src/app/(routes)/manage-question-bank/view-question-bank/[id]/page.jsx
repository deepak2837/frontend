...
import { RestartAlt as RestartAltIcon, Edit as EditIcon } from '@mui/icons-material';
...
useEffect(() => {
  ...
  const exam = queryParams.get("exam") || "all";
  const search = queryParams.get("search") || "";

  setFilters({ subject, difficulty, exam });
  ...
  const handleRemoveQuestions = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/api/v1/question-bank/remove-questions/${params.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (data.success) {
        ...
        // Remove selected questions from the local state
          ...
          <CardContent>
            ...
            <DataGrid
              ...
              onRowSelectionModelChange={(newSelection) => {
                setSelectedQuestions(newSelection); // Ensure this updates the state
              }}
              selectionModel={selectedQuestions} // Bind the selection model to the state
              components={{
                Toolbar: GridToolbar,
              }}
              autoHeight
            ...
            {selectedQuestions.length > 0 && (
              <Button
                ...
                sx={{ mt: 2 }}
              >
                Delete Selected Questions ({selectedQuestions.length})
              </Button>
            )}
          </CardContent>
        </Card>
