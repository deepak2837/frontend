...
const BlogCard = ({ post }) => {
  ...
  return (
    <div
      className="rounded-2xl shadow-lg hover:shadow-2xl transition duration-300 overflow-hidden border border-white/30 flex flex-col group relative"
      style={{
      ...
      >
        <div className="relative w-full aspect-[16/9] overflow-hidden">
          ...
          <div className="absolute top-2 right-2 flex gap-1 z-20">
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs sm:text-[10px] font-semibold bg-gradient-to-r from-[#FE6B8B] to-[#FF8E53] text-white border border-white/30 shadow backdrop-blur-md">
        ...
        <div className="p-3 sm:p-2 flex-grow flex flex-col justify-between relative z-10 mt-[-8px]">
          <div className="rounded-xl bg-white/50 backdrop-blur-md border border-white/30 px-3 py-2 sm:px-2 sm:py-1 shadow-sm">
            <h2 className="text-base sm:text-sm font-extrabold text-[#FE6B8B] mb-1 sm:mb-0.5 drop-shadow-lg line-clamp-2">{post.subtitle}</h2>
            
            <p className="text-gray-700 text-xs sm:text-[11px] mb-1 line-clamp-1">{post.summary}</p>
            <div className="flex flex-wrap gap-1 mb-0.5">
              {post.tags && post.tags.slice(0, 2).map((tag) => (
                <span key={tag} className="px-2 py-0.5 rounded-full text-xs sm:text-[10px] font-semibold bg-gradient-to-r from-[#FE6B8B] to-[#FF8E53] text-white border border-white/20 shadow-sm select-none">
                  #{tag}
                </span>
              ))}
            </div>
